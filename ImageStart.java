...
public class ImageStart {
	...
	public static void main(String[] args) throws IOException {
		...
		if (count==1) {			
			for (int i=0;i<files.length;i++) {
				if (files[i].startsWith("color_mask.")){
					file_color_mask = new File(files[i]);
				}
			}			
		}	
		//------------------------------------------------------------------------------------------------------------------------------
			...
			for(double x=bounds.getLeft_up(b).getX();x<bounds.getRight_bottom(b).getX()+1;x++) {
				for(double y=bounds.getLeft_up(b).getY();y<bounds.getRight_bottom(b).getY()+1;y++) {					
					...
					case 1:
						...
						break;
					case 2:
						c=getColorInterpolationX(x_in, y_in, image_color_mask);
						break;
					case 3:
						c=getColorInterpolationY(x_in, y_in, image_color_mask);
						break;
					case 4:
						c=getColorInterpolationXY(x_in, y_in, image_color_mask);
						break;					
					}					
